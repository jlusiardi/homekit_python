...
class AccessoryRequestHandler(BaseHTTPRequestHandler):
    ...
    def _post_pair_verify(self):
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M1:
            ...
            ios_device_curve25519_pub_key_bytes = bytes(d_req[1].data)
            ...
            d_res.append(tlv8.Entry(TLV.kTLVType_State, TLV._M2))
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M3:
            ...
            d_res.append(tlv8.Entry(TLV.kTLVType_State, TLV._M4))
    ...
    def _post_pairings(self):
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M1 \
                and d_req[1].type_id == TLV.kTLVType_Method and d_req[1].data == TLV._AddPairing:
            ...
            if not server_data.is_peer_admin(ios_device_pairing_id):
                self.send_error_reply(TLV._M2, TLV._kTLVError_Authentication)
                self.log_error('error in step #2: admin bit')
                return

            additional_controller_pairing_identifier = d_req[2].data
            additional_controller_LTPK = d_req[3].data
            additional_controller_permissions = d_req[4].data
            ...
            if registered_controller_LTPK is not None:
                ...
                if registered_controller_LTPK != additional_controller_LTPK:
                    ...
                    # 3.a)
                    self.send_error_reply(TLV._M2, TLV._kTLVError_Unknown)
                    return
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M1 \
                and d_req[1].type_id == TLV.kTLVType_Method and d_req[1].data == TLV._RemovePairing:
            ...
            server_data.remove_peer(d_req[2].data)
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M1 \
                and d_req[1].type_id == TLV.kTLVType_Method and d_req[1].data == TLV._ListPairings:
            ...
            tmp = [tlv8.Entry(TLV.kTLVType_State, TLV._M2)]
    ...
    def send_error_reply(self, state, error):
        ...
        :param state: The state as in TLV._M1, TLV._M2, ...
        :param error: The error code as in TLV._kTLVError_*
        :return: None
        """
        d_res = [
    ...
    def _post_pair_setup(self):
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M1:
            ...
            # 1) Check if paired
            if self.server.data.is_paired:
                self.send_error_reply(TLV._M2, TLV._kTLVError_Unavailable)
                return

            # 2) Check if over 100 attempts
            if self.server.data.unsuccessful_tries > 100:
                self.log_error('to many failed attempts')
                self.send_error_reply(TLV._M2, TLV._kTLVError_MaxTries)
                return

            # 3) Check if already in pairing
            if False:
                self.send_error_reply(TLV._M2, TLV._kTLVError_Busy)
                return
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M3:
            ...
            ios_pub_key = int.from_bytes(d_req[1].data, "big")
            ...
            # 2) verify ios proof
            ios_proof = int.from_bytes(d_req[2].data, "big")
            if not server.verify_clients_proof(ios_proof):
                d_res.append(tlv8.Entry(TLV.kTLVType_State, TLV._M4))
                d_res.append(tlv8.Entry(TLV.kTLVType_Error, TLV._kTLVError_Authentication))
        ...
        if d_req[0].type_id == TLV.kTLVType_State and d_req[0].data == TLV._M5:
            ...
            if decrypted_data is False:
                d_res.append((TLV.kTLVType_State, TLV._M6))
                d_res.append((TLV.kTLVType_Error, TLV._kTLVError_Authentication))

                self.send_error_reply(TLV._M6, TLV._kTLVError_Authentication)
            ...
            ios_device_pairing_id = d_req_2[0].data  # should be TLV.kTLVType_Identifier
            ios_device_ltpk = d_req_2[1].data  # should be TLV.kTLVType_PublicKey
            ios_device_info = ios_device_x + ios_device_pairing_id + ios_device_ltpk

            # 5) verify signature
            ios_device_sig = d_req_2[2].data  # should be [TLV.kTLVType_Signature
            ...
            self.server.publish_device()
            d_res = [
                tlv8.Entry(TLV.kTLVType_State, TLV._M6),
