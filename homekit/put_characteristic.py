...
def get_format(accessories_data, aid, iid):
    format = None
    for d in accessories_data:
...
if __name__ == '__main__':
    ...
    characteristics = [(c[0].split('.')[0], c[0].split('.')[1], c[1]) for c in args.characteristics]
    print(characteristics)
    pairing.put_characteristics(characteristics)

    # session = Session.create_from_file(args.file)
    # accessories = session.get_from_pairing_data('accessories')
    #
    ...
    #     if not accessories or not get_format(accessories, aid, iid):
    #         # nope, so get it via /accessories and save it
    #         response = session.get('/accessories')
    #         data = json.loads(response.read().decode())
    #         accessories = data['accessories']
    #         session.set_in_pairing_data('accessories', accessories)
    #         session.save_pairing_data(args.file)
    #     # after loading the accessories data the aid.iid should be there...
    #     characteristic_type = get_format(accessories, aid, iid)
    ...
    # response = session.put('/characteristics', body)
