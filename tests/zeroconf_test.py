...
class TestZeroconf(unittest.TestCase):
    ...
    def test_find_with_device(self):
        ...
        info = ServiceInfo('_hap._tcp.local.', 'foo1._hap._tcp.local.', addresses=[socket.inet_aton('127.0.0.1')],
    ...
    def test_discover_homekit_devices(self):
        ...
        info = ServiceInfo('_hap._tcp.local.', 'foo2._hap._tcp.local.', addresses=[socket.inet_aton('127.0.0.1')],
                           port=1234, properties=desc, weight=0, priority=0)
        zeroconf.unregister_all_services()
        zeroconf.register_service(info, allow_name_change=True)
    ...
    def test_discover_homekit_devices_missing_c(self):
        ...
        info = ServiceInfo('_hap._tcp.local.', 'foo3._hap._tcp.local.', addresses=[socket.inet_aton('127.0.0.1')],
                           port=1234, properties=desc, weight=0, priority=0)
        zeroconf.unregister_all_services()
        zeroconf.register_service(info, allow_name_change=True)
    ...
    def test_discover_homekit_devices_missing_md(self):
        ...
        info = ServiceInfo('_hap._tcp.local.', 'foo4._hap._tcp.local.', addresses=[socket.inet_aton('127.0.0.1')],
                           port=1234, properties=desc, weight=0, priority=0)
        zeroconf.unregister_all_services()
        zeroconf.register_service(info, allow_name_change=True)
