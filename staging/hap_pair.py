...
if __name__ == '__main__':
    ...
    resp_tlv = TLV.decode_bytes(resp_tlv[0][1])
    logging.debug('response TLV: %s', resp_tlv)
    logging.debug('step: %s', resp_tlv[0])
    logging.debug('pub key: %s', resp_tlv[1])
    logging.debug('salt: %s', resp_tlv[2])
