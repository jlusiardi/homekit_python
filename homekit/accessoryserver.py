...
class AccessoryServerData:
    ...
    def remove_peer(self, pairing_id: str):
        del self.data['peers'][pairing_id]
        self._save_data()
...
class AccessoryRequestHandler(BaseHTTPRequestHandler):
    ...
    def _post_pair_verify(self):
        ...
        d_req.assert_has(Types.kTLVType_State, 'No state available')
#        if d_req[0].type_id == Types.kTLVType_State and d_req[0].data == Steps.M1:
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M1:
            ...
            ios_device_curve25519_pub_key_bytes = bytes(d_req.first_by_id(Types.kTLVType_PublicKey).data)
            ...
            # 5) sub tlv
            sub_tlv = [
                tlv8.Entry(Types.kTLVType_Identifier, self.server.data.accessory_pairing_id_bytes),
                tlv8.Entry(Types.kTLVType_Signature, accessory_signature)
            ]
            ...
            d_res.append(tlv8.Entry(Types.kTLVType_State, Steps.M2))
            d_res.append(tlv8.Entry(Types.kTLVType_PublicKey, accessory_spk))
            d_res.append(tlv8.Entry(Types.kTLVType_EncryptedData, tmp))
        ...
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M3:
            ...
            # 2) decrypt
            encrypted = d_req.first_by_id(Types.kTLVType_EncryptedData).data
            decrypted = chacha20_aead_decrypt(bytes(), session_key, 'PV-Msg03'.encode(), bytes([0, 0, 0, 0]),
                                              encrypted)
            if decrypted is False:
                self.send_error_reply(Steps.M4, Errors.kTLVError_Authentication)
            ...
            d1.assert_has(Types.kTLVType_Identifier, 'no identifier found')
            d1.assert_has(Types.kTLVType_Signature, 'no signature found')

            # 3) get ios_device_ltpk
            ios_device_pairing_id = d1.first_by_id(Types.kTLVType_Identifier).data
            ...
            ios_device_sig = d1.first_by_id(Types.kTLVType_Signature).data
            ...
            d_res.append(tlv8.Entry(Types.kTLVType_State, Steps.M4))
    ...
    def _post_pairings(self):
        ...
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M1 \
                and d_req.first_by_id(Types.kTLVType_Method).data == Methods.AddPairing:
            ...
            if not server_data.is_peer_admin(ios_device_pairing_id):
                self.send_error_reply(Steps.M2, Errors.kTLVError_Authentication)
                self.log_error('error in step #2: admin bit')
                return

            additional_controller_pairing_identifier = d_req.first_by_id(Types.kTLVType_Identifier).data
            additional_controller_LTPK = d_req.first_by_id(Types.kTLVType_PublicKey).data
            additional_controller_permissions = d_req.first_by_id(Types.kTLVType_Permissions).data
            ...
            if registered_controller_LTPK is not None:
                ...
                if registered_controller_LTPK != additional_controller_LTPK:
                    ...
                    # 3.a)
                    self.send_error_reply(Steps.M2, Errors.kTLVError_Unknown)
                    return
        ...
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M1 \
                and d_req.first_by_id(Types.kTLVType_Method).data == Methods.RemovePairing:
            ...
            server_data.remove_peer(d_req.first_by_id(Types.kTLVType_Identifier).data)
            ...
            #    if session['ios_device_pairing_id'] == d_req[Types.kTLVType_Identifier]:
            #        session['handler'].close_connection = True
            #
            # if self.server.sessions[self.session_id]['ios_device_pairing_id'] == d_req[Types.kTLVType_Identifier]:
            #    self.close_connection = True
            return

        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M1 \
                and d_req.first_by_id(Types.kTLVType_Method).data == Methods.ListPairings:
            ...
            tmp = [tlv8.Entry(Types.kTLVType_State, Steps.M2)]
            for index, pairing_id in enumerate(server_data.peers):
                tmp.append(tlv8.Entry(Types.kTLVType_Identifier, pairing_id.encode()))
                tmp.append(tlv8.Entry(Types.kTLVType_PublicKey, server_data.get_peer_key(pairing_id.encode())))
                user = Types.kTLVType_Permission_RegularUser
                if server_data.is_peer_admin(pairing_id.encode()):
                    user = Types.kTLVType_Permission_AdminUser
                tmp.append(tlv8.Entry(Types.kTLVType_Permissions, user))
                if index + 1 < len(server_data.peers):
                    tmp.append(tlv8.Entry(Types.kTLVType_Separator, bytes(0)))
    ...
    def send_error_reply(self, state, error):
        """
        Send an error reply encoded as Types.kTLVType
        :param state: The state as in Steps.M1, Steps.M2, ...
        :param error: The error code as in Types.kTLVType_kTLVError_*
        :return: None
        """
        d_res = [
    ...
    def _post_pair_setup(self):
        ...
        d_req = tlv8.decode(self.body, expectations)
        ...
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M1:
            ...
            # 1) Check if paired
            if self.server.data.is_paired:
                self.send_error_reply(Steps.M2, Errors.kTLVError_Unavailable)
                return

            # 2) Check if over 100 attempts
            if self.server.data.unsuccessful_tries > 100:
                self.log_error('to many failed attempts')
                self.send_error_reply(Steps.M2, Errors.kTLVError_MaxTries)
                return

            # 3) Check if already in pairing
            if False:
                self.send_error_reply(Steps.M2, Errors.kTLVError_Busy)
                return
            ...
            d_res.append(tlv8.Entry(Types.kTLVType_State, Steps.M2))
            d_res.append(tlv8.Entry(Types.kTLVType_PublicKey, SrpServer.to_byte_array(public_key)))
            d_res.append(tlv8.Entry(Types.kTLVType_Salt, SrpServer.to_byte_array(salt)))
            self._send_response_tlv8(d_res)
        ...
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M3:
            ...
            ios_pub_key = int.from_bytes(d_req.first_by_id(Types.kTLVType_PublicKey).data, "big")
            ...
            # 2) verify ios proof
            ios_proof = int.from_bytes(d_req.first_by_id(Types.kTLVType_Proof).data, "big")
            if not server.verify_clients_proof(ios_proof):
                d_res.append(tlv8.Entry(Types.kTLVType_State, Steps.M4))
                d_res.append(tlv8.Entry(Types.kTLVType_Error, Errors.kTLVError_Authentication))
            ...
            d_res.append(tlv8.Entry(Types.kTLVType_State, Steps.M4))
            d_res.append(tlv8.Entry(Types.kTLVType_Proof, SrpServer.to_byte_array(accessory_proof)))
        ...
        if d_req.first_by_id(Types.kTLVType_State).data == Steps.M5:
            ...
            # 2) decrypt and test
            encrypted_data = d_req.first_by_id(Types.kTLVType_EncryptedData).data
            ...
            if decrypted_data is False:
                d_res.append((Types.kTLVType_State, Steps.M6))
                d_res.append((Types.kTLVType_Error, Errors.kTLVError_Authentication))

                self.send_error_reply(Steps.M6, Errors.kTLVError_Authentication)
                self.log_error('error in step #6 %s %s', d_res, self.server.sessions)
                return

            d_req_2 = tlv8.decode(bytes(decrypted_data))
            ...
            d_req_2.assert_has(Types.kTLVType_Identifier, 'no device identifier found')
            d_req_2.assert_has(Types.kTLVType_PublicKey, 'no public key found')
            d_req_2.assert_has(Types.kTLVType_Signature, 'no signature found')
            # should be Types.kTLVType_Identifier
            ios_device_pairing_id = d_req_2.first_by_id(Types.kTLVType_Identifier).data
            # should be Types.kTLVType_PublicKey
            ios_device_ltpk = d_req_2.first_by_id(Types.kTLVType_PublicKey).data
            ios_device_info = ios_device_x + ios_device_pairing_id + ios_device_ltpk

            # 5) verify signature
            # should be Types.kTLVType_Signature
            ios_device_sig = d_req_2.first_by_id(Types.kTLVType_Signature).data
            ...
            sub_tlv = [
                tlv8.Entry(Types.kTLVType_Identifier, self.server.data.accessory_pairing_id_bytes),
                tlv8.Entry(Types.kTLVType_PublicKey, accessory_ltpk.to_bytes()),
                tlv8.Entry(Types.kTLVType_Signature, accessory_signature)
            ]
            sub_tlv_b = tlv8.encode(sub_tlv)
            ...
            self.server.publish_device()
            d_res = [
                tlv8.Entry(Types.kTLVType_State, Steps.M6),
    ...
    def _send_response_tlv8(self, d_res, close=False, status=HttpStatusCodes.OK):
        result_bytes = tlv8.encode(d_res)
